...
# 
#       while(1){
#       off_set = (abs(K_y-P_y)/2);// divided into 2 pairs of values (n=abs(K_y-P_y)), and sort them with a kernel function using n/2 blocksl
#       Q_x = K_x + off_set;
#       Q_y = K_y - off_set;
#       if ((Q_y>=y_start && Q_y>=0) && (Q_x<=(Z+x_start)<=dim_B) && ((Q_y == (Z+y_start) || Q_y == dim_A) || (Q_x==x_start || Q_x==0) || A_sh[Q_y - y_start] > B_sh[Q_x -1 - x_start]) ){
#           __syncthreads();
